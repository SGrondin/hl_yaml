...
let default_read_file filename =
  Lwt_io.with_file ~flags:[ O_RDONLY; O_NONBLOCK ] ~mode:Input filename Lwt_io.read
...
let make_options ?get_env_var ?get_file ?config_path_relative_to ?file_path_relative_to ?enable_includes
  ?enable_imports ?allow_unused_anchors ?validate_config_path ?process_scalar_tag () =
  {
    ...
    get_file = Option.value get_file ~default:default_read_file;
    ...
    validate_config_path = Option.value validate_config_path ~default:(fun _ -> Lwt.return_true);
  }
...
module YAML = struct
  ...
  type state = {
    refs: (yaml Lwt.t * int) StringTable.t;
    env_vars: string StringTable.t;
    files: string Lwt.t StringTable.t;
    configs: yaml Lwt.t StringTable.t;
    options: options;
  }
  ...
  let resolve_references state (yaml : yaml) =
    ...
    in
    let rec loop : yaml -> yaml Lwt.t = function
          ...
          (function
            | `O { m_members = [ ((`Scalar { tag = Some "!IF_DEF"; _ } as left), right) ]; _ }
             ...
             |`O { m_members = [ ((`Scalar { value = "<<"; _ } as left), right) ]; _ }
              ...
              | (true, _), `A { s_members; _ } -> Lwt_list.map_s loop s_members
              | (false, _), _ -> Lwt.return_nil
              | (true, _), yaml -> Lwt.return [ yaml ] )
            | yaml ->
      ...
      | `Scalar scalar as original ->
        (match scalar with
        | { tag = None; _ } as x -> Lwt.return (`Scalar { x with anchor = None })
        | { tag = Some tag; value; _ } -> (
          ...
          | Some p, _ -> (
            ...
            | `YAML x -> x )
          | None, "!ENV" -> get_env_var value |> make_scalar |> Lwt.return
          | None, "!FILE" -> get_file value >|= make_scalar
          | None, "!ENV_FILE" -> get_env_var value |> get_file >|= make_scalar
          | None, "!ENV_STR" -> get_env_var value |> make_key |> make_scalar |> Lwt.return
          | None, "!CONFIG" when state.options.enable_imports ->
            let* is_allowed = state.options.validate_config_path value in
            if is_allowed then get_config value else failwithf "YAML !CONFIG was denied for: %s" value ()
    ...
    Lwt.catch
      ...
      (function
        | Failure msg -> Lwt.return_error msg
        | exn -> Lwt.return_error (Printexc.to_string exn))
    >|= function
  ...
  let of_string ?(options = default_options) str =
    ...
    | Error (`Msg message) -> Lwt.return (Error message)
end
